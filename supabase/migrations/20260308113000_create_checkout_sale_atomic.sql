-- ============================================================================
-- MIGRATION: CREATE ATOMIC CHECKOUT RPC
-- ============================================================================

-- ============================================================================
-- 1) ESQUEMA MINIMO PARA IDEMPOTENCIA
-- ============================================================================

alter table public.sales
  add column if not exists checkout_ref text null;

create unique index if not exists idx_sales_checkout_ref_unique
  on public.sales(checkout_ref)
  where checkout_ref is not null;

-- ============================================================================
-- 2) RPC: create_checkout_sale
-- Reglas:
-- - Una sola venta para multiples items
-- - Operacion atomica
-- - Vendedor resuelto por auth.uid()
-- - Idempotencia por checkout_ref
-- ============================================================================
...
begin
  ...
  loop
    ...
    if not found then
      raise exception 'Item %: el producto % no existe.', v_line_no, v_product_id;
    end if;

    if v_product.is_active is false then
      raise exception 'Item %: el producto % esta inactivo.', v_line_no, v_product_id;
    end if;

    if v_unit_price_sold < v_product.minimum_price then
      raise exception 'Item %: unit_price_sold (%) no puede ser menor a minimum_price (%).',
        v_line_no, v_unit_price_sold, v_product.minimum_price;
    end if;

    v_payload_mode := nullif(trim(coalesce(v_item->>'mode', '')), '');
    if v_payload_mode is not null and v_payload_mode <> v_product.inventory_mode then
      raise exception 'Item %: mode no coincide con inventory_mode del producto.', v_line_no;
    end if;

    if v_product.inventory_mode = 'quantity' then
      if coalesce(trim(v_item->>'quantity'), '') = '' then
        raise exception 'Item %: quantity es obligatorio para productos quantity.', v_line_no;
      end if;
      ...
      exception
        when others then
          raise exception 'Item %: quantity invalido.', v_line_no;
      end;

      if v_quantity_numeric <= 0 or v_quantity_numeric <> trunc(v_quantity_numeric) then
        raise exception 'Item %: quantity debe ser entero mayor que 0.', v_line_no;
      end if;
      ...
      if coalesce(trim(v_serialized_unit_id_text), '') <> '' then
        raise exception 'Item %: serialized_unit_id no aplica para productos quantity.', v_line_no;
      end if;
      ...
      v_subtotal := round((v_quantity * v_unit_price_sold)::numeric, 2);
      ...
      v_serialized_unit_id_text := coalesce(v_item->>'serialized_unit_id', v_item->>'serializedUnitId');
      if coalesce(trim(v_serialized_unit_id_text), '') = '' then
        raise exception 'Item %: serialized_unit_id es obligatorio para productos serial.', v_line_no;
      end if;
      ...
      if coalesce(trim(v_item->>'quantity'), '') <> '' then
        ...
        if v_quantity_numeric <> 1 then
          raise exception 'Item %: quantity debe ser 1 para productos serial.', v_line_no;
        end if;
      end if;
      ...
      select exists (
        select 1
        from pg_temp.tmp_checkout_items t
        where t.serialized_unit_id = v_serialized_unit_id
      )
      into v_has_duplicate_serial;

      if v_has_duplicate_serial then
        raise exception 'Item %: serialized_unit_id repetido en el payload.', v_line_no;
      end if;

      select *
      into v_serialized_unit
      from public.serialized_units su
      where su.id = v_serialized_unit_id
      for update;

      if not found then
        raise exception 'Item %: la unidad serializada % no existe.', v_line_no, v_serialized_unit_id;
      end if;

      if v_serialized_unit.product_id <> v_product_id then
        raise exception 'Item %: la unidad serializada no pertenece al producto indicado.', v_line_no;
      end if;

      if v_serialized_unit.status <> 'available' then
        raise exception 'Item %: la unidad serializada no esta disponible.', v_line_no;
      end if;
